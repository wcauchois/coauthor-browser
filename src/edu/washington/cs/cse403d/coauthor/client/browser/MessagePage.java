...
/**
 * A page analogous to a JOptionPane in conventional Swing applications. This
 * page will display a message, accompanied by some descriptive text and
 * optionally an icon (currently three types are supported: error, warning, and
 * info) and/or some buttons (some combination of "OK", "Cancel", and
 * "Go back").
 * 
...
public class MessagePage extends BrowserPage {
	...
	public static final int ERROR = 1, // A red X indicating an error has
			// occurred
	...
	public static final int GO_BACK = 1 << 0, OK = 1 << 1, CANCEL = 1 << 2;

	/**
	 * Construct a MessagePane with the specified title and descriptive text, no
	 * icon or button.
	 * 
	...
	/**
	 * Construct a MessagePane with the specified title, descriptive text, icon
	 * associated with the type parameter (which must be one of ERROR, INFO,
	 * WARNING, or 0), and one or more buttons. Use an ActionListener to
	 * determine when one of the buttons has been pressed.
	 * 
	 * @see #addActionListener(ActionListener)
	 * @param type
	 *            one of ERROR, INFO, WARNING, or 0.
	 * @param buttons
	 *            some combination of GO_BACK, OK, or CANCEL bitwise OR'd
	 *            together.
	 */
	public MessagePage(int type, String title, String details, int buttons) {
		...
		if (type == ERROR)
			icon = resMan.loadImageIcon("error.png");
		else if (type == INFO)
			icon = resMan.loadImageIcon("info.png");
		else if (type == WARNING)
			icon = resMan.loadImageIcon("warning.png");

		setLayout(new BorderLayout());
		JPanel messagePane = new JPanel();
		messagePane.setLayout(new BoxLayout(messagePane, BoxLayout.Y_AXIS));
		messagePane.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Color.BLACK),
		...
		if ((buttons & GO_BACK) == GO_BACK)
			buttonList.add(new JButton("Go back"));
